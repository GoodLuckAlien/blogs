...
大家好，我是 👽 ，接下来会出一个新系列，**React v18新特性解读**，主要针对新特性的**产生背景**，**功能介绍**，和**原理分析**等几个方面，勇于做第一个吃螃蟹的人。希望支持我的朋友可以**点赞**，**转发**，**再看**，关注一波公众号，持续分享前端技术硬文。
...
常规模式下，先把外部数据 external Data 通过 selector 选择器把组件需要的数据映射到 state | props 上。这真是完成了一步，接下来需要 subscribe 订阅外部数据源的变化，如果发生变化，那么还需要自身去强制更新 forceUpdate 。下面两幅图表示数据注入和数据订阅更新。

典型的外部数据源就是 redux 中的 store ，redux 是如何把 Store 中的 state ，安全的变成组件的 state 的。 

或许我可以用一段代码来表示从 react-redux 改变到视图更新的流程.
...
如上是通过 useMutableSource ，这样让代码减少了 APP 内部组件代码，代码健壮性提升，一定程度上也降低了耦合。接下来让我们全方面认识一下这个 V18 的新特性。
...
* 第二个参数：一个函数，函数的返回值作为数据源的版本号，这里需要注意⚠️的是，要保持数据源和数据版本号的一致性，就是数据源变化了，那么数据版本号就要变化，一定程度上有点像 `immutable` （不可变性）。可以理解为数据版本号是证明数据源唯一性的标示。
...
* [useMutableSource RFC](https://github.com/reactjs/rfcs/blob/main/text/0147-use-mutable-source.md)